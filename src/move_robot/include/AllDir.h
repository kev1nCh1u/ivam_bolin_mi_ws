class alldir: public Move_Robot
{
	public:
		...
		bool qrcodemove();
		void get_vy(float& vy, float qrcode_A);
...
alldir::alldir(char* dev_name, int Baudrate):Move_Robot(dev_name, Baudrate)
{
  ...
  qrcode_tag = 0;
	...
	ini_way_theta = 0.0;
	omni_first_heading = 0.0;
	mission_dis_3m = false;
	...
	//commandSubscriber_ = node_.subscribe("Command", 5, &alldir::commandCallback, this);
	qrcodeSubscriber_=node_.subscribe("qrcode", 5, &alldir::qrcodeCallback, this);
...
alldir::~alldir()
{
	std::cout<<"alldir delete"<<std::endl;
		if(receive_thread_)
			delete receive_thread_;
}
void alldir::get_vy(float& vy, float qrcode_A)
{
		if(fabs(qrcode_A) < 1.5)
		{
				if(qrcode_y > 0)
				{
						vy = -0.02;
...
bool alldir::qrcodemove()
{
	static int mode = 0;
	static bool min_angle = false;
	float dis_err = sqrt(pow(qrcode_x,2)+pow(qrcode_y,2));

	std::cout<<"qrcode_x "<<qrcode_x <<std::endl;
	std::cout<<"qrcode_y "<<qrcode_y <<std::endl;
	float trans_angle = 0.0;
	float move_vx = 0.0020;
	float move_vy = 0.0020;
	float move_w = 0.0020;

	if(qrcode_direction == 0)
	{
			trans_angle = qrcode_A;
	}
	else if(qrcode_direction == 1)
	{
			trans_angle = qrcode_A - 90.0;
			if(trans_angle < 0.0)trans_angle = trans_angle + 360.0;
	}
	else if(qrcode_direction == 2)
	{
			trans_angle = qrcode_A + 180.0;
			if(trans_angle >= 360.0)trans_angle = trans_angle - 360.0;
	}
	else if(qrcode_direction == 3)
	{
			trans_angle = qrcode_A + 90.0;
			if(trans_angle >= 360.0)trans_angle = trans_angle - 360.0;
	}
	...
	std::cout<<"qrcode_th = "<<qrcode_th<<std::endl;
	std::cout<<"dis_err "<< dis_err<<std::endl;

	if(qrcode_tag!=0)
	{
			if(dis_err <= 0.85 && min_angle == true)
			{
				mode = 0;
				min_angle = false;
				float vx = 0.0;
				...
				sendreceive.Package_testWheel_encoder(vx, vy, W_rw, 0, command_buf);
				SendPackage(command_buf);
				return true;
			}
			else
			{
				...
				float W_rw = 0.0;
				int angle_mode = 0;
				if(fabs(qrcode_th) <= 0.785)angle_mode = 0;//-0.785-0.785
				else if(fabs(qrcode_th) >= 2.355)angle_mode = 1;//-2.335- -2.335
				else if(qrcode_th < -0.785 && qrcode_th > -2.355)angle_mode = 2;//-0.785- -2.335
				else if(qrcode_th > 0.785 && qrcode_th < 2.355)angle_mode = 3;//0.785 - 2.355
				switch(mode)
				{
						case 0:
						  std::cout<<"0============="<<std::endl;
		          ...
		          if(fabs(qrcode_y) <= 1)
						...
						switch(angle_mode)
						{
								case 0:
									...
									break;
								case 1:
									if(fabs(qrcode_th)>3.139)
									{
										std::cout<<"B finish============="<<std::endl;
										mode = 0;
										vx = 0.0;
										vy = 0.0;
										W_rw = 0.0;
									...
									break;
								case 2:
									if((qrcode_th>-1.571 && qrcode_th <= -1.57) || (qrcode_th<-1.569 && qrcode_th >= -1.57))
									{
									...
									break;
								case 3:
									if((qrcode_th>1.569 && qrcode_th <= 1.57) || (qrcode_th<1.571 && qrcode_th>=1.57))
									{
...
void alldir::qrcodeCallback(const move_robot::qrcode& msg)
{
	qrcode_x = (float)msg.x/10;
...
void alldir::timerCallback(const ros::TimerEvent& event)
{
	...
	if(isReveice_joystick){
		std::cout<<"isReveice_joystick"<<std::endl;
		v_buf = 0.04;
...
int type,id;
	...
	switch (state) {
						...
						if(now_path_index+1 < A_misson[now_A_misson].sub_missonPath.size())
						{
							...
							if(trafficGO_recv.id==id)
								if(traffic_go_1)//traffic_go 改
								{
									...
									OBS_limilMode = false;
									traffic_send = false;
									traffic_go_1 = false;//traffic_go 改
								}
						}
					}
				}
		...
		case P_STATE_QRCODE:  //LOADING TIME
				...
				if((int)qrcode_timeout == qrcode_timer)
				{
						ready_path_index =0;
						A_misson[ready_path_index].ALL_pathnode.clear();
						A_misson.clear();
	...
	if(!isReSet){
		...
		if(isInitial){
			...
			//讓所有輪子歸0
				...
				if(kinematics_model == "omni" || kinematics_model == "omni_2"){
					omni_first_heading = robot_pos.z();
					isFInish = false;
					isInitial = true;
					target_ind = 0;
					pre_angular_error = 0;
					...
					int now_index = 0;
					float odom_v = 0.5;
					int target_pos_index = A_misson[ready_path_index].sub_missonPath[subpath_index].sub_missonPath_subPoint.size()-1;
					target_pos = A_misson[ready_path_index].sub_missonPath[subpath_index].sub_missonPath_subPoint[target_pos_index];
    ...
    /////////////////////////
		...
		if(startmove)
		{
			...
			if(fabs(m_error) < 0.1 )
			{
				...
				else if(kinematics_model == "omni" || kinematics_model == "omni_2")
				{   //OBS_limilMode = false;
		...
		//===前85% 的導航====
		int First_tracking = Path_size * 0.3;
		std::cout<<"AA"<<std::endl;
		...
		if(now_index < First_tracking ){
			...
			if(kinematics_model == "diff"){
				...
				if(fabs(cmd_angular_velocity) >= move_w_max){
					...
					else cmd_angular_velocity = -1*move_w_max;
				}
			...
			else if(kinematics_model == "omni")
			{
					...
					angular_velocity_error = omni_first_heading - robot_pos.z();
					if(fabs(angular_velocity_error) > M_PI){
						if(angular_velocity_error > 0) angular_velocity_error = angular_velocity_error - 2*M_PI ;
						else                           angular_velocity_error = angular_velocity_error + 2*M_PI;
					}
			...
			else if(kinematics_model == "omni_2"){
			...
			if(kinematics_model == "omni" || kinematics_model == "omni_2"){
				...
				if(mission_dis_3m)cmd_velocity = 0.5;
				...
				cmd_velocity = V_sub_target;
				if(mission_dis_3m)cmd_velocity = 0.1;
				Vx = cmd_velocity;
				Vy = 0;
				W_rw = cmd_angular_velocity;
		...
		if(confirm_last_diff_angle  && dis_error <= 0.01 && !Endangle )
		{
		...
		if(Endangle == true && fabs(angular_error) <= 0.001)
		{
				isFInish = true;
				cmd_velocity = 0;
				W_rw = 0;
...
std::cout<<"see 5  "<<std::endl;
	...
	// void alldir::commandCallback(const move_robot::setmap_ctr& command)
	// {
	...
	{//==============change=============
		...
		{ROS_INFO("turn_left");
			...
			sendreceive.Package_testWheel_encoder(0, 0, 0.35, 0, command);
	...
	void alldir::Stop()
	{
		...
		if(isInitial){
			...
			if(!isStop){
				...
				sendreceive.Package_testWheel_encoder(start_vx, start_vy, 0, 0, command);
