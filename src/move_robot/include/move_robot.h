...
//	ros::Subscriber Clear_ObsModeSubscriber_;
    ...
    float tracking_kd;
		...
		std::string map_name;
		int qrcode_direction;
		float qrcode_timeout;
    //int Car_packMode;
	...
	bool v_stop ;
	bool traffic_send;
	bool traffic_go_1;//用來保護traffic_go = 1時不要被洗掉
	//電梯
	bool ElevatorGO;
	int floor;
...
{
		node_.shutdown();
		std::cout<<"Move_Robot close"<<std::endl;
}
...
{
    std::cout<< "IdTypeCallback "<<std::endl;
...
int Move_Robot::SetLocalpar(std::string file_buf)
{
    ...
    if(!fin.is_open())
    {
        ...
        std::getline(fin, map_name_temp);
				...
				std::string qrcode_direction_temp;
				std::getline(fin, qrcode_direction_temp);
        std::getline(fin, qrcode_direction_temp);
				qrcode_direction = std::atoi(qrcode_direction_temp.c_str());

				std::string qrcode_timeout_temp;
				std::getline(fin, qrcode_timeout_temp);
				std::getline(fin, qrcode_timeout_temp);
				qrcode_timeout = std::atof(qrcode_timeout_temp.c_str()) / 0.1;
    }
